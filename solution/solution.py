...
def compare_models(prompt: str) -> dict:
    ...
    estimated_output_tokens = len(gpt4o_text.split()) / 0.75
...
def run_assistant(
    ...
    history, num_turns, total_tokens, total_cost = [], 0, 0, 0.0
    while True:
        # Phải kiểm tra trước khi đọc input.
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # Chỉ giữ ba lượt user–assistant gần nhất.
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def truncate_text(text: Any, limit: int = 40) -> str:
        text_str = str(text)
    ...
    for result in results:
        rows.append([
            truncate_text(result.get("prompt", "")),
            truncate_text(result.get("gpt4o_response", "")),
            truncate_text(result.get("mini_response", "")),
    ...
    output_lines = [format_row(headers), separator]
...
if __name__ == "__main__":
    ...
    response, latency = call_openai_mini("Hãy kể cho tôi một sự thật thú vị về Việt Nam", temperature=1.5)  # demo chạy thật, cần OPENAI_API_KEY
    print(response, latency)
    # print(os.getenv("OPENAI_API_KEY"))
    # response, latency = call_openai_mini("Hello world")  # demo chạy thật, cần OPENAI_API_KEY
    # print(response, latency)
